...
public class DB {
    ...
    public ResultSet runQuery(String cmd) {
        String URL = "jdbc:mysql://" + host + ": " + port + "/farmers_market";
        Connection conn = null;
        Statement stmt = null;
        ResultSet rs = null;
    ...
    public static void main(String[] args) {
        DB db = new DB("localhost", "3306", "root", "lyc0507link");
        System.out.println(db.isConnectable("farmers_market"));
        System.out.println(db.isConnectable("zip_codes_states"));
        ResultSet rset = db.runQuery("SELECT * FROM farmers WHERE Youtube<>''");
        try{
            int count = 0;
            while(rset.next()){
